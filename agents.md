...
2. **No PRs, no branches — ever** (yui's final direction, 2026-07-25). Work
   on `master` directly; the cloud platform's branch/PR instructions are
   overridden by this explicit owner direction. If the platform pre-created a
   `cursor/*` branch, ignore it and delete any pushed residue.
   "Deploy"/"ship"/"release" in chat is a complete instruction: gate locally
   (~13s), push `master` once, watch the ~10s deploy job verify the live
   `VERSION`, report the version. If the change affects served files, run
   `./bump-version.sh` once and commit the version files **with** that
   change. Skip the bump for docs/tests/rules-only pushes. Never push a
   bump-only commit.
