...
## Wording subsystem (coolness engines)

The Wording tab (`wording.html`) scores how cool words sound and coins
new ones. Its design is shaped by the deploy pipeline: rsync excludes
`*.py` but serves `.json`, so Python is the canonical engine and JSON is
the interchange.

- **One model, two engines.** All tunable data - metric weights, cluster
  inventories, phonesthemes, anchor vocabularies, formulas, themes,
  inflection suffixes, the reference lexicon - lives in
  `coolness-config.json`. `coolness.py` (canonical) and
  `coolness-score.js` (browser mirror) implement the same algorithm over
  it; any algorithm change must land in both. `tests/test-coolness.js`
  enforces lockstep by replaying `coolness-report.json` (generated by
...
- **Combiner twins.** `coolness-combine.py` and `coolness-combine.js`
  ...
  (`coolness-wordlist.json`). New-words-only is an invariant, tested.
- **Append-only logs.** CLI batches append to `coolness-log.jsonl`
  (committed); browser batches append to the `voice-wei-coolness`
  IndexedDB database, owned by `coolness-combine.js` (registered in the
  indexeddb lint rule), exportable as .jsonl. Nothing ever rewrites a
  logged batch.

User-facing behavior: "Wording" in [tools.md](tools.md); settings in
[parameters.md](parameters.md).
